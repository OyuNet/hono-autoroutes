...
`
  ...
  console.log('SMOKE_RESPONSE_FS=', txt)
  ...
  const entries = {
    ...
    '/virtual/routes/index/route.ts': async () => ({
      default: new (await import('hono')).Hono().get('/', (c) => c.text('should-not-see')),
    }),
  }
  ...
  console.log('SMOKE_RESPONSE_ENTRIES=', txt2)
  ...
  const entriesLast = {
    '/virtual/routes/index/route.mjs': () => import(routeFile),
    '/virtual/routes/index/route.ts': async () => ({
      default: new (await import('hono')).Hono().get('/', (c) => c.text('last-wins')),
    }),
  }
